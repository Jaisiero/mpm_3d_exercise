...
presets = [
    ...
    [
        ...
        CubeVolume(ti.Vector([0.35, 0.35, 0.35]), ti.Vector([0.25, 0.25, 0.25]), SNOW),
        CubeVolume(ti.Vector([0.05, 0.6, 0.05]), ti.Vector([0.25, 0.25, 0.25]), JELLY),
        CubeVolume(ti.Vector([0.05, 0.05, 0.05]), ti.Vector([0.25, 0.25, 0.25]),
            RIGID),
    ],
]
preset_names = [
...
material_colors = [(0.1, 0.6, 0.9), (0.93, 0.33, 0.23), (1.0, 1.0, 1.0), (0.2, 0.8, 0.0)]
...
def show_options():
    ...
    with gui.sub_window("Options", 0.05, 0.45, 0.2, 0.4) as w:
        ...
        if not use_random_colors:
            ...
            material_colors[RIGID] = w.color_edit_3("rigid color", material_colors[RIGID])
